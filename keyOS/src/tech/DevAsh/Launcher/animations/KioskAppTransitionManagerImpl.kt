...
import tech.DevAsh.Launcher.findInViews
...
class KioskAppTransitionManagerImpl(context: Context) : LauncherAppTransitionManagerImpl(context), KioskPreferences.OnPreferenceChangeListener {
    ...
    private val animationType by context.KioskPrefs.StringBasedPref("pref_animationType",
            AnimationType.DefaultAnimation(), { },
            AnimationType.Companion::fromString,
            AnimationType.Companion::toString) { registerRemoteAnimations() }

    init {
        Utilities.getKioskPrefs(launcher).addOnPreferenceChangeListener(this, *prefsToListen)
    ...
    override fun destroy() {
        super.destroy()
        Utilities.getKioskPrefs(launcher).removeOnPreferenceChangeListener(this, *prefsToListen)
    }
    ...
    override fun createLauncherResumeAnimation(anim: AnimatorSet) {
        ...
        val background = KioskLauncher.getLauncher(launcher).background
        background.blurAlphas.getProperty(KioskBackgroundView.ALPHA_INDEX_TRANSITIONS).value = 1f
        val blurAlpha = ObjectAnimator.ofFloat<InvertedMultiValueAlpha.InvertedAlphaProperty>(background.blurAlphas.getProperty(
                KioskBackgroundView.ALPHA_INDEX_TRANSITIONS),
    ...
    public override fun getLauncherContentAnimator(isAppOpening: Boolean): Pair<AnimatorSet, Runnable> {
        ...
        val background = KioskLauncher.getLauncher(launcher).background
        background.blurAlphas.getProperty(KioskBackgroundView.ALPHA_INDEX_TRANSITIONS).value = blurAlphaFrom
        val blurAlpha = ObjectAnimator.ofFloat<InvertedMultiValueAlpha.InvertedAlphaProperty>(background.blurAlphas.getProperty(
                KioskBackgroundView.ALPHA_INDEX_TRANSITIONS),
    ...
    companion object {
        ...
        private const val TAG = "KioskTransition"
